...
void CMiscHacks::ChatSpamInterwebz()
{
	...
	if (wasSpamming)
	{
		...
		if (useSpace)
		{
			change_name ("\xe1\xab\xa8 Rekthook.xyz");
			useSpace = !useSpace;
		}
		else
		{
			change_name("\xe1\xab\xa5 Rekthook.xyz");
			useSpace = !useSpace;
		}
	}
...
void CMiscHacks::ChatSpamRegular()
{
	...
	if (Menu::Window.MiscTab.OtherTeamChat.GetState())
	{
		SayInTeamChat("Boost service by Rekthook.xyz");
	}
	else
	{
		SayInChat("Derank service by Rekthook.xyz");
	}
