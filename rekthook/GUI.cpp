...
bool CGUI::DrawWindow(CWindow* window)
{
	...
	Render::Clear(window->m_x + 8, window->m_y + 1 + 27, window->m_iWidth - 4 - 12, window->m_iHeight - 2 - 8 - 26, Color(0, 0, 0, 0.07));

	//Tab
	Render::Clear(window->m_x + 8, window->m_y + 1 + 27, window->m_iWidth - 4 - 12, 29, Color(25, 25, 25, 155));
	int TabCount = window->Tabs.size();
	if (TabCount) // If there are some tabs
	{
		int TabSize = (window->m_iWidth - 4 - 12) / TabCount;
		for (int i = 0; i < TabCount; i++)
		{
			...
			{
				Render::Clear(window->m_x + 8 + (i*TabSize), window->m_y + 1 + 27, TabSize, 29, Color(25, 25, 25, 155));
			}
